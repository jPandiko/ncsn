...
class AnnealRunner():
    ...
    def sample_images(self,scorenet,*,num_samples: int,batch_size: int = 64,n_steps_each: int = 100,step_lr: float = 2e-5,):
      ...
      # torch sigma schedule on correct device
      sigmas = torch.tensor(
        np.exp(np.linspace(
      ...
      with torch.no_grad():
        while global_idx < num_samples:
            ...
            # invert logit transform if used
            if self.config.data.logit_transform:
                x = torch.sigmoid(x)
    ...
    """
    Load checkpoint and run sampling to produce individual PNG files.
    Controlled by CLI args: --num_samples --sample_batch_size --n_steps_each --step_lr
    """
    def sample_pipeline(self):
      ...
      self.sample_images(
        ...
        n_steps_each=self.args.n_steps_each,
