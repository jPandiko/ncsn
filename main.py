...
def parse_args_and_config():
    ...
    parser.add_argument('--step_lr', type=float, default=2e-5, help='Base step size for Langevin dynamics')
    ...
    # Logging / checkpoint directory is always derived from --run and --doc
    args.log = os.path.join(args.run, 'logs', args.doc)

    # We treat test/sample/inpaint as eval-like modes:
    # they should load config from args.log/config.yml (trained run config).
    ...
    # setup logging + save config only for training
    if not is_eval:
        ...
        logger.handlers = []  # avoid duplicated handlers in notebooks/re-runs
        ...
        logger = logging.getLogger()
        logger.handlers = []  # avoid duplicated handlers
        logger.addHandler(handler1)
        logger.setLevel(level)
...
def main():
    ...
    logging.info("Writing log file to %s", args.log)
    logging.info("Exp instance id = %s", os.getpid())
    logging.info("Exp comment = %s", args.comment)
    logging.info("Config =")
    print(">" * 80)
    print(config)
    print("<" * 80)
    ...
    try:
        ...
        if args.mode == 'train':
            ...
            # You said you renamed your sampling entrypoint to avoid interfering with test().
